...
public class FolderDAO {
  ...
  public TreeNode getSubTreeFolder(int userID, int folderID) throws SQLException {
    ...
    String query = "WITH RECURSIVE FolderHierarchy AS (SELECT * FROM Folder WHERE owner_id = ? AND folder_id = ? UNION SELECT f.* FROM Folder f INNER JOIN FolderHierarchy fh ON f.parent_folder_id = fh.folder_id) SELECT * FROM FolderHierarchy ORDER BY depth ASC";
  ...
  public void deleteFolder(int userID, int folderID) throws SQLException {
    ...
    try {
      try (PreparedStatement lock = connection.prepareStatement("SELECT user_id FROM User WHERE user_id=? FOR UPDATE")) {
        lock.setInt(1, userID);
        try (ResultSet rows = lock.executeQuery()) {
          if (!rows.next()) throw new SQLException("Owner not found");
        }
      }
      String query = "WITH RECURSIVE descendants AS ("
      + "SELECT folder_id,0 AS distance FROM Folder WHERE owner_id=? AND folder_id=? AND depth>0 "
      + "UNION ALL SELECT f.folder_id,h.distance+1 FROM Folder f JOIN descendants h "
      + "ON f.parent_folder_id=h.folder_id WHERE f.owner_id=?) "
      + "SELECT folder_id FROM descendants ORDER BY distance DESC";
      ArrayList<Integer> ids = new ArrayList<>();
      try (PreparedStatement statement = connection.prepareStatement(query)) {
        statement.setInt(1, userID); statement.setInt(2, folderID); statement.setInt(3, userID);
        try (ResultSet rows = statement.executeQuery()) {
          while (rows.next()) ids.add(rows.getInt(1));
        }
      }
      try (PreparedStatement documents = connection.prepareStatement("DELETE FROM Document WHERE owner_id=? AND folder_id=?");
          PreparedStatement folders = connection.prepareStatement("DELETE FROM Folder WHERE owner_id=? AND folder_id=?")) {
        for (int id : ids) {
          documents.setInt(1, userID); documents.setInt(2, id); documents.executeUpdate();
          folders.setInt(1, userID); folders.setInt(2, id); folders.executeUpdate();
        }
      }
  ...
  public int getLastFolderID(int userID) throws SQLException {
    ...
    try (PreparedStatement statement = connection.prepareStatement(query)) {
      ...
      try (ResultSet result = statement.executeQuery()) {
        ...
        int id = result.getInt("max_folder_id");
